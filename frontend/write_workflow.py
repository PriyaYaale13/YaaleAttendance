    ...
    try {
      ...
      if (response.ok) {
        ...
        const mapped = data.map(rec => {
          ...
          return {
            id: rec.id, // Actual DB ID
            reportId: OT-,
            empName: rec.name,
            period: rec.date,
            totalOt: ${rec.submittedOtHours || 0} hrs,
            totalAmount: $,
            currentStage: stage,
            status: wfStatus,
            originalRecord: rec,
          ...
          <table className="table table-hover align-middle mb-0">
            <thead className="table-light">
              <tr>
                ...
                <th className="px-4 py-3 text-uppercase text-firo-muted small fw-semibold">Total OT / Amount</th>
              ...
              {filteredWorkflows.map((wf) => (
                <tr key={wf.id}>
                  ...
                  <td className="px-4 py-3">
                    <div className="fw-bold text-firo-dark">{wf.totalOt}</div>
                  ...
                  <td className="px-4 py-3">
                    <span className={classNames('badge rounded-pill px-3 py-2 fw-medium border', {
                      ...
                      {wf.status === 'Pending' ? Pending  : wf.status}
                    </span>
                  </td>
                ...
                <div className="modal-body p-4">
                  <div className="bg-firo-bg rounded-3 p-3 mb-4">
                    <div className="row g-2 text-firo-dark small">
                      ...
                      <div className="col-6"><span className="text-firo-muted">Total OT:</span> <span className="fw-bold">{selectedReport?.totalOt}</span></div>
...
'''
with open(r'd:\ATM-WMS\frontend\src\components\ApprovalWorkflow\index.jsx', 'w', encoding='utf-8') as f:
    f.write(code)
