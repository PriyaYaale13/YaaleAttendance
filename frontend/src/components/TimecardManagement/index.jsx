...
const TimecardManagement = () => {
  ...
  const fetchTimecards = async () => {
    try {
      const response = await fetch('http://localhost:8000/timecards/');
      if (response.ok) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (emp) {
          ...
          let presentDays = 0;
          try {
            const attRes = await fetch('http://localhost:8000/attendance/');
            if (attRes.ok) {
  ...
  const handleDownload = (tc) => {
    window.open(`http://localhost:8000/timecards/download/${tc.id}`, '_blank');
  };

  const handleView = (tc) => {
    window.open(`http://localhost:8000/timecards/view/${tc.id}`, '_blank');
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this timecard?')) {
      try {
        const response = await fetch(`http://localhost:8000/timecards/${id}`, {
          method: 'DELETE'
        });
        if (response.ok) {
          fetchTimecards();
