...
const DocumentManagement = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const response = await fetch('http://localhost:8000/employee-documents/');
      if (response.ok) {
  ...
  const handleDownload = (doc) => {
    window.open(`http://localhost:8000/employee-documents/download/${doc.id}`, '_blank');
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this document?')) {
      try {
        const response = await fetch(`http://localhost:8000/employee-documents/${id}`, {
          method: 'DELETE'
        });
        if (response.ok) {
          fetchDocuments();
