...
def create_user(user: schemas.SystemUserCreate, db: Session = Depends(get_db)):
    ...
    db_user = models.SystemUser(
        ...
        secondary_contact_no=user.secondaryContactNo,
        role=user.role,
        system_date_format=user.systemDateFormat
    )
    db.add(db_user)
    create_audit_log(db, "System User", "Created Record: Create User", "Users", "Unknown", "-", "-", "Action performed via API", "info")
    db.commit()
    db.refresh(db_user)
    ...
    return schemas.SystemUserResponse(
        ...
        midName=db_user.mid_name,
        lastName=db_user.last_name,
        gender=db_user.gender,
        birthDate=db_user.birth_date,
        userName=db_user.user_name,
        email=db_user.email,
        secondaryEmail=db_user.secondary_email,
        contactNo=db_user.contact_no,
        secondaryContactNo=db_user.secondary_contact_no,
        role=db_user.role,
        systemDateFormat=db_user.system_date_format
    )
...
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    for user in users:
        result.append(schemas.SystemUserResponse(
            ...
            secondaryContactNo=user.secondary_contact_no,
            role=user.role,
            systemDateFormat=user.system_date_format
        ))
    return result
...
def update_user(user_id: int, user: schemas.SystemUserUpdate, db: Session = Depends(get_db)):
    ...
    create_audit_log(db, "System User", "Updated Record: Update User", "Users", "Unknown", "-", "-", "Action performed via API", "info")
    db.commit()
    db.refresh(db_user)
    return schemas.SystemUserResponse(
        id=db_user.id,
...
def delete_user(user_id: int, db: Session = Depends(get_db)):
    ...
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    db.delete(db_user)
    create_audit_log(db, "System User", "Deleted Record: Delete User", "Users", "Unknown", "-", "-", "Action performed via API", "info")
    db.commit()
    return {"ok": True}
